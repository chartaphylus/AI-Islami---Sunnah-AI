...
export default function AIBanner() {
    return (
        <div className="relative overflow-hidden bg-gray-900 dark:bg-black rounded-2xl md:rounded-[2rem] p-4 md:py-6 md:px-8 shadow-2xl border border-gray-800">
            {/* Background Effects */}
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6 text-center md:text-left">
                ...
                <Link
                    href="/ai"
                    className="group relative inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2 px-5 md:py-3.5 md:px-7 rounded-xl transition-all shadow-lg shadow-emerald-600/30 hover:shadow-emerald-500/50 hover:-translate-y-1 active:scale-95"
                >
                    <MessageSquare className="w-4 h-4 md:w-5 md:h-5 group-hover:scale-110 transition-transform" />
                    <span className="text-sm md:text-base">Mulai Chat</span>
                </Link>
            </div>
        </div>);
}
